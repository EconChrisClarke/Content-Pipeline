...
(async function () {
  ...
  async function markdownToDoc(markdown, uploadImageFn, onProgress) {
    ...
    const footnoteDefs = [];
    // The app's own script editor appends an optional {sm}/{md}/{lg} marker
    // right after an image's closing ")" for the Small/Medium/Large presets
    // (no marker = Full). This used to break detection entirely here, since
    // a sized image line ends in "}" not ")" -- so it fell through to being
    // parsed as a plain paragraph instead of an image, AND the size itself
    // was never read (every image was hardcoded to Substack's 'normal'
    // width regardless of what was chosen). Both are fixed by parsing the
    // marker first, then mapping it to a resizeWidth.
    //
    // The mapping below scales proportionally off the resizeWidth (728)
    // already used for Full-size images -- matching the same relative
    // Small=35% / Medium=60% / Large=85% / Full=100% proportions the app's
    // own CSS uses for these presets in its script editor (see .size-sm/
    // .size-md/.size-lg). This hasn't been visually confirmed against a
    // real Substack post the way the footnote schema was -- worth a quick
    // check after deploying (post one image at each size and eyeball it)
    // and adjusting these numbers if any size looks off.
    const IMAGE_SIZE_RE = /\{(sm|md|lg)\}$/;
    const FULL_RESIZE_WIDTH = 728;
    const SIZE_RESIZE_WIDTH = {
      sm: Math.round(FULL_RESIZE_WIDTH * 0.35),
      md: Math.round(FULL_RESIZE_WIDTH * 0.60),
      lg: Math.round(FULL_RESIZE_WIDTH * 0.85)
    };
    function parseImageBlock(trimmed) {
      let t = trimmed;
      let size = 'full';
      const m = IMAGE_SIZE_RE.exec(t);
      if (m) { size = m[1]; t = t.slice(0, -m[0].length); }
      if (!(t.startsWith('![') && t.endsWith(')'))) return null;
      const cut = t.lastIndexOf('](');
      if (cut === -1) return null;
      return { caption: t.slice(2, cut), path: t.slice(cut + 2, -1), size };
    }
    const imageBlocks = blocks.filter(b => parseImageBlock(b.trim()) !== null);
    let imgDone = 0;
    for (const block of blocks) {
      const trimmed = block.trim();
      if (!trimmed) continue;
      const imgInfo = parseImageBlock(trimmed);
      if (imgInfo) {
        const { caption, path, size } = imgInfo;
        imgDone++;
        if (onProgress) onProgress(imgDone, imageBlocks.length);
        const url = await uploadImageFn(path);
        const resizeWidth = SIZE_RESIZE_WIDTH[size] || FULL_RESIZE_WIDTH;
        const content = [{
          type: 'image2',
          attrs: {
            src: url, srcNoWatermark: null, fullscreen: false, imageSize: 'normal',
            height: 819, width: 1456, resizeWidth, bytes: null,
